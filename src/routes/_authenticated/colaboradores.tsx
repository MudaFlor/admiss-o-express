...
function ColaboradoresPage() {
  ...
  async function onCreate(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      toast.success("Colaborador criado. Compartilhe o link.");
    } catch (err) {
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Convide um novo colaborador e gere um link para coleta de dados e documentos via WhatsApp.
          </p>
        </div>
        <Dialog open={open} onOpenChange={(v) => { setOpen(v); if (!v) setCreated(null); }}>
          ...
          <DialogContent>
            <DialogHeader>
              <DialogTitle>{created ? "Link gerado" : "Novo Colaborador"}</DialogTitle>
              <DialogDescription>
                {created
                  ? "Compartilhe o link abaixo com o colaborador pelo WhatsApp. Ele poderá preencher os dados e enviar os documentos."
                  : "Informe os dados básicos. Um link único será gerado para envio via WhatsApp."}
              </DialogDescription>
            </DialogHeader>
            {!created ? (
          ...
          <Table>
            ...
            <TableBody>
              {q.isLoading ? (
                ...
                <TableRow><TableCell colSpan={4} className="text-center text-sm text-muted-foreground">Nenhum colaborador ainda. Clique em "Novo Colaborador".</TableCell></TableRow>
              ) : (
                q.data!.map((c) => (
                  <TableRow key={c.id}>
                    <TableCell>
